...
Deploy, ayrı bir `deploy.yml` değil — `.github/workflows/test.yml` içindeki `build_and_deploy` job'ı: `test` job'ına `needs:` ile bağlı, yalnızca `main`'e push'ta ve testler geçtiyse çalışır, hosting ile birlikte `firestore.rules`/`firestore.indexes.json` dosyalarını da production'a uygular. `environment: production` ile bir onay kapısı arkasındadır (bkz. `docs/RUNBOOK.md` §6). Hosting canlıya "kör" yazılmaz: build önce 1 saat ömürlü geçici bir preview channel'a yüklenir, orada `scripts/hostingSaglikKontrolu.ts` (build'e gömülü commit SHA'sı + ana JS/manifest/sw doğrulaması) geçerse canlıya terfi eder, canlıda aynı kontrol tekrarlanır ve geçmezse hosting bir önceki sürüme **otomatik geri alınır** (bkz. `docs/RUNBOOK.md` §1). **Bilinen sınırlama**: production tek ortamdır — ayrı bir staging yoktur (preview channel aynı production Firestore/Auth'a bağlanır, yalnızca HTTP düzeyinde doğrulama için kullanılır, uygulama mantığı orada test edilmez).
